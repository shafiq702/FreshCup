...
App.directive('sidebar', function($state, $rootScope, $localStorage){
    return {
        restrict: 'E',
        scope: {},
        templateUrl: 'assets/views/partials/base_sidebar.html',
        link: function(scope, element){
            jQuery('[data-toggle="nav-submenu"]', element).on('click', function(e){
                // Get link
                var link = jQuery(this);
...
App.directive('baseHeader', function(userFactory, $state, $rootScope, $localStorage){
  return {
    restrict: 'E',
    scope: {},
...
App.directive('subHeader', function(){
  return {
    restrict: 'E',
    templateUrl: 'assets/views/partials/base_sub_header.html',
  }
})
...
// Credit Card
// By adding the attribute 'data-js-card'
// App.directive('creditCard', function () {
//     return {
//         restrict: 'A',
//         link: function(scope, element) {
//           App.initHelpers('appear');
//           console.log(jQuery('.js-card-form').card)
...
App.directive('jsSummernote', function () {
    return {
        link: function (scope, element, attrs) {
            ...
            if(scope.post.articleBody){
                jQuery(element).summernote('code', scope.post.articleBody);
            }
        }
    };
});
...
App.directive('jsSimplemde', function () {
    return {
        link: function (scope, element, attrs) {
            var simplemde = new SimpleMDE({ element: jQuery(element)[0] });
            if (scope.post){
              simplemde.value(scope.post.articleBody);
            }

            simplemde.codemirror.on("change", function(){
            	scope.textareaValue = simplemde.value()
            });
