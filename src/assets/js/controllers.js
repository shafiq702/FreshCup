...
App.controller('BMFormCtrl', ['$scope', '$localStorage', '$window', '$sce', '$stateParams',
    function ($scope, $localStorage, $window, $sce, $stateParams) {
      var url = "https://docs.google.com/forms/d/e/"+$stateParams.formID+"/viewform?embedded=true"
      $scope.formURL = $sce.trustAsResourceUrl(url)
    }
]);

App.controller('homeCtrl', ['$scope', '$localStorage', '$window',
    function ($scope, $localStorage, $window) {

    }
]);

App.controller('newMemberRegistrationCtrl', ['$scope', '$localStorage', '$window',
    function ($scope, $localStorage, $window) {

    }
]);

App.controller('AdminCoursesCtrl', ['$scope', '$localStorage', '$window', 'courses', 'courseFactory',
    function ($scope, $localStorage, $window, courses, courseFactory, $state) {
      $scope.courses = courses
      $scope.deleteCourse = function(courseId){
        courseFactory.deleteCourse(courseId)
...
App.controller('AdminUnitsCtrl', ['$scope', '$localStorage', '$window', 'units', 'courseFactory', '$state', '$stateParams',
    function ($scope, $localStorage, $window, units, courseFactory, $state, $stateParams) {
      $scope.units = units
      $scope.courseId = $stateParams.courseId
      $scope.deleteUnit = function(unitId){
        courseFactory.deleteUnit(unitId)
...
App.controller('aboutUsCtrl', ['$scope', '$localStorage', '$window', 'mailchimpFactory', '$state', '$stateParams',
    function ($scope, $localStorage, $window, mailchimpFactory, $state, $stateParams) {
    }
]);

App.controller('AdminLessonsCtrl', ['$scope', '$localStorage', '$window', 'lessons', 'courseFactory', '$state', '$stateParams',
    function ($scope, $localStorage, $window, lessons, courseFactory, $state, $stateParams) {
      $scope.lessons = lessons
      $scope.unitId = $stateParams.unitId
      $scope.deletelesson = function(lessonId){
        courseFactory.deleteLesson($stateParams.unitId,lessonId)
        .then(function(result){
          if(result){
            $state.reload()
          }
...
App.controller('AdminProductsCtrl', ['$scope', '$localStorage', '$window', 'books', 'bookFactory',
    function ($scope, $localStorage, $window, books, bookFactory, $state) {
      $scope.books = books
...
App.controller('AdminCustomersCtrl', ['$scope', '$localStorage', '$window',
    function ($scope, $localStorage, $window, $state) {

    }
]);

App.controller('BooksCtrl', ['$scope', '$localStorage', '$window', 'books','cartFactory',
    function ($scope, $localStorage, $window, books, cartFactory) {
      $scope.books = books
      $scope.addToCart = function(product){
        var item = {title: product.title, price: product.price, quantity: 1}
        cartFactory.addToCart(item)
      }
    }
]);

App.controller('CoursesCtrl', ['$scope', '$localStorage', '$window', 'courses',
    function ($scope, $localStorage, $window, courses, $state) {
      $scope.courses = courses
    }
]);

App.controller('SingleCourseCtrl', ['$scope', '$localStorage', '$window', 'course', 'units', '$state',
    function ($scope, $localStorage, $window, course, units, $state) {
      $scope.course = course
      $scope.units = units
    }
]);

App.controller('BookDetailCtrl', ['$scope', '$localStorage', '$window', 'book','cartFactory',
    function ($scope, $localStorage, $window, book, cartFactory) {
      $scope.book = book
...
App.controller('CheckoutCtrl', ['$scope', '$localStorage', '$window', 'cartFactory','userFactory','orderFactory',
    function ($scope, $localStorage, $window, cartFactory,userFactory,orderFactory) {
      $scope.products = $localStorage.cartItems
      $scope.total = cartFactory.totalPrice()

      $scope.increaseQuantity = function(product){
        cartFactory.increaseQuantity(product)
      }

      $scope.decreaseQuantity = function(product){
        cartFactory.decreaseQuantity(product)
      }

      $scope.removeItem = function(product){
        cartFactory.removeItem(product)
      }
...
App.controller('AdminAddCoursesCtrl', ['$scope', '$localStorage', '$window','courseFactory','$state',
    function ($scope, $localStorage, $window, courseFactory, $state) {
...
App.controller('AdminAddUnitsCtrl', ['$scope', '$localStorage', '$window','courseFactory', '$state', '$stateParams',
    function ($scope, $localStorage, $window, courseFactory, $state, $stateParams) {
      $scope.addUnit = function(unit){
        unit.course = $stateParams.courseId
        courseFactory.createUnit(unit)
        .then(function(unit){
          if(unit){
            $state.go('adminUnits', {courseId: $stateParams.courseId})
          }
          return err;
        })
        .catch(function(err){
...
App.controller('AdminAddLessonsCtrl', ['$scope', '$localStorage', '$window','courseFactory', '$state', '$stateParams',
    function ($scope, $localStorage, $window, courseFactory, $state, $stateParams) {
      $scope.addLesson = function(lesson){
        lesson.unit = $stateParams.unitId
        courseFactory.createLesson(lesson)
        .then(function(lesson){
          if(lesson){
            $state.go('adminLessons',{unitId: $stateParams.unitId})
          }
          return err;
        })
        .catch(function(err){
...
App.controller('AdminUpdateUnitsCtrl', ['$scope', '$localStorage', '$window','courseFactory','unit', '$state', '$stateParams',
    function ($scope, $localStorage, $window, courseFactory, unit, $state, $stateParams) {
      $scope.unit = unit
      $scope.updateUnit = function(unit){
        courseFactory.updateUnit(unit)
        .then(function(result){
          $state.go('adminUnits',{courseId: $scope.unit.course})
        })
        .catch(function(err){
          return err;
        })
...
App.controller('AdminUpdateLessonsCtrl', ['$scope', '$localStorage', '$window','courseFactory','lesson', '$state', '$stateParams',
    function ($scope, $localStorage, $window, courseFactory, lesson, $state, $stateParams) {
      $scope.lesson = lesson
      $scope.updateLesson = function(lesson){
        courseFactory.updateLesson(lesson)
        .then(function(result){
          if(result){
            $state.go('adminLessons',{unitId: lesson.unit})
          }
          return leson
        })
        .catch(function(err){
...
App.controller('AdminAddBooksCtrl', ['$scope', '$localStorage', '$window','bookFactory',
    function ($scope, $localStorage, $window, bookFactory, $state) {
...
App.controller('AdminUpdateCoursesCtrl', ['$scope', '$localStorage', '$window', 'course','courseFactory', '$state',
    function ($scope, $localStorage, $window, course, courseFactory, $state) {
      $scope.course = course
...
App.controller('AdminUpdateBooksCtrl', ['$scope', '$localStorage', '$window', 'book','bookFactory',
    function ($scope, $localStorage, $window, book, bookFactory, $state) {
      $scope.book = book
...
App.controller('AddRouteCtrl', ['$scope', '$localStorage','$location','$window','routeFactory','$state','route','users',
    function ($scope, $localStorage,$location,$window,routeFactory,$state,route,users) {
        $scope.location = $location.$$path;
        $scope.route = route;
        $scope.users = users;
        $scope.submitRouteForm = function(route){
            if($location.$$path == '/add/route'){
                routeFactory.createRoute(route)
                    .then(function(route){
                        $state.go('routesTable')
                        $scope.route = {};
                        return route
                    })
                    .catch(function(err){
                ...
                routeFactory.updateRoute(route)
                    .then(function(result){
                        $scope.route = {};
                        $state.go('routesTable')
                        return store
...
App.controller('AddStoreCtrl', ['$scope', '$localStorage','$location','$window','storeFactory','$state','store','routes',
    function ($scope, $localStorage,$location,$window,storeFactory,$state,store,routes) {
            $scope.location = $location.$$path;
            $scope.store = store;
            $scope.routes = routes;
            $scope.submitStoreForm = function(store){
                if($location.$$path == '/add/store'){
                    storeFactory.createStore(store)
                        .then(function(store){
                            $state.go('storesTable')
                            $scope.store = {};
                            return store
                        })
                        .catch(function(err){
                    ...
                    storeFactory.updateStore(store)
                        .then(function(result){
                            $scope.store = {};
                            $state.go('storesTable')
                            return store
...
App.controller('profileStoreCtrl', ['$scope', '$localStorage','$location', '$window', 'commentFactory', '$state', 'store', 'comments',
    function ($scope, $localStorage,$location, $window, commentFactory, $state, store, comments) {
        $scope.store = store;
        $scope.comments = comments[0];
        $scope.addComment = function (comment) {
            comment.author = $localStorage.user._id;
            comment.store = store._id;
            commentFactory.createComment(comment)
                .then(function(res){
                    $state.reload()
                })
                .catch(function(err){
...
// Blog Controller
App.controller('BlogCtrl', ['$scope', '$localStorage', '$window', 'posts',
...
// Blog Controller
App.controller('TutorialsCtrl', ['$scope', '$localStorage', '$window', 'posts',
    function ($scope, $localStorage, $window, posts) {
      $scope.posts = posts
    }
]);

// Single Tutorial controller
App.controller('SingleTutorialCtrl', ['$scope', '$localStorage', '$window', '$location', '$sce', 'post',
    function ($scope, $localStorage, $window, $location, $sce, post) {
      $scope.post = post
      $scope.post.articleVideo = $sce.trustAsResourceUrl($scope.post.articleVideo)
      $scope.pagePath = $location.path();
    }
]);

// Single Post controller
App.controller('SinglePostCtrl', ['$scope', '$localStorage', '$window', '$location', '$sce', 'post',
    function ($scope, $localStorage, $window, $location, $sce, post) {
      $scope.post = post
      $scope.post.articleVideo = $sce.trustAsResourceUrl($scope.post.articleVideo)
      $scope.pagePath = $location.path();
...
App.controller('TutorialTablesCtrl', ['$scope', '$localStorage', '$window','posts',
    function ($scope, $localStorage, $window, posts) {
      $scope.posts = posts;
...
// Tables DataTables Controller
App.controller('StoresTableCtrl', ['$scope', '$state', '$localStorage','$location', '$window','stores', 'storeFactory',
    function ($scope, $state, $localStorage,$location, $window, stores, storeFactory) {
        $scope.stores = stores;
        $scope.removeStore = function(storeId){
            swal({
                title: 'Are you sure?',
                text: 'You are about to permenantly about to delete this store!',
                type: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d26a5c',
...
App.controller('RoutesTableCtrl', ['$scope', '$state', '$localStorage','$location', '$window','routes', 'routeFactory',
    function ($scope, $state, $localStorage,$location, $window, routes, routeFactory) {
        $scope.routes = routes;
        console.log(routes)
        $scope.removeRoute = function(routeId){
            swal({
                title: 'Are you sure?',
                text: 'You are about to permenantly about to delete this route!',
                type: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d26a5c',
                confirmButtonText: 'Yes, delete store!',
...
App.controller('FormsEditorsCtrl', ['$scope', '$localStorage', '$window', 'blogFactory', '$state',
    function ($scope, $localStorage, $window, blogFactory, $state) {

        $scope.publishPost = function(){
          $scope.post.articleBody = $scope.content
          $scope.post.isDraft = false;
          blogFactory.createPost($scope.post)
          .then(function(res){
            $state.go('tablesDatatables')
            return res
          })
          .catch(function(err){
            console.log(err)
...
// Form Editors Controller
App.controller('FormsEditorsUpdateCtrl', ['$scope', '$state', '$localStorage', '$window', 'blogFactory','post',
    function ($scope, $state, $localStorage, $window, blogFactory, post) {
      $scope.post = post
        ...
        $scope.updatePost = function(){
          $scope.post.articleBody = $scope.content
          $scope.post.isDraft = false;
          blogFactory.updatePost($scope.post)
          .then(function(res){
            $state.go('tablesDatatables')
            return res
          })
